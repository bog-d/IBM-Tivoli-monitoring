...
<head>
	...
	<title>Просмотр логов действий в форме "Настройка интеграции с СТП"</title>
...
<body>
    ...
    // top header
    $title = "Просмотр логов действий, выполненных из формы \"Настройка интеграции с СТП\"";
    $links = array ("");
	...
	// logs output
    echo "<p align='right' class='red_message'>Внимание! Поле \"Номер заявки\" добавлено с 26.07.2017 г. До этой даты номер заявки записывался в поле \"Примечание\".</p>";
	...
	echo "<table width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"8\">";
        ...
        echo "<tr>";
            foreach ($titles as $index) {
                $sel = "SELECT distinct ".($index == "TIMESTAMP" ? "date(TIMESTAMP) as TIMESTAMP" : "\"{$index}\"")." FROM DB2INST1.PFR_ACTIONS_LOG WHERE SERVICE_NAME = '{$service}' ORDER BY \"{$index}\" ".($index == "TIMESTAMP" ? "DESC" : "ASC");
        ...
        $sel = "SELECT * FROM DB2INST1.PFR_ACTIONS_LOG WHERE SERVICE_NAME = '{$service}' {$filters_string} ORDER BY TIMESTAMP DESC";
        $stmt_TBSM = db2_prepare($connection_TBSM, $sel);
        $result = db2_execute($stmt_TBSM);
        while ($row = db2_fetch_assoc($stmt_TBSM)) {
            echo "<tr>";
